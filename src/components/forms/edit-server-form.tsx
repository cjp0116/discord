'use client'

import { useActionState } from "react"
import { updateServer } from "@/lib/actions/servers"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { FormField, FormError } from "@/components/ui/form"
import { FadeIn } from "@/components/ui/fade-in"

interface EditServerFormProps {
  serverId: string
  initialData: {
    name: string
    description: string
  }
} 

export function EditServerForm({ serverId, initialData }: EditServerFormProps) {
  const [state, formAction, isPending] = useActionState(
    async (prevState:any, formData:FormData) => {
      return await updateServer(serverId, formData as FormData)
    },
    null
  );
  const action = formAction;

  return (
    <FadeIn className="w-full max-w-md">
      <Card className="animate-in slide-in-from-bottom-4 duration-500 shadow-xl border-border/50">
        <CardHeader>
          <CardTitle>Edit Server</CardTitle>
        </CardHeader>

        <CardContent>
          <form action={action} className="space-y-4">
            <FadeIn delay={100}>
              <FormField error={state?.errors?.name?.[0]}>
                <Label htmlFor="name">Server Name</Label>
                <Input
                  id="name"
                  name="name"
                  type="text"
                  placeholder="Enter server name"
                  required
                  disabled={isPending}
                  className="transition-all duration-200 focus:scale-[1.02] focus:ring-2 focus:ring-primary/20"
                  defaultValue={initialData.name}
                />
              </FormField>
            </FadeIn>

            <FadeIn delay={200}>
              <FormField error={state?.errors?.description?.[0]}>
                <Label htmlFor="description">Description (Optional)</Label>
                <Input
                  id="description"
                  name="description"
                  type="text"
                  placeholder="Enter server description"
                  disabled={isPending}
                  className="transition-all duration-200 focus:scale-[1.02] focus:ring-2 focus:ring-primary/20"
                  defaultValue={initialData.description}
                />
              </FormField>
            </FadeIn>

            <FadeIn delay={300}>
              <Button type="submit" className="w-full bg-gradient-primary text-white hover:shadow-lg transition-all duration-300" disabled={isPending} loading={isPending}>
                Update Server
              </Button>
            </FadeIn>

            {state?.errors && !state.success && (
              <FadeIn delay={400}>
                <FormError message="Please check the errors above and try again." />
              </FadeIn>
            )}
          </form>
        </CardContent>
      </Card>
    </FadeIn>
  )
}